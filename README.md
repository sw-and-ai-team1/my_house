...
**RandomForestRegressor는 앙상블 기계학습 알고리즘**

### 핵심 개념:

- Random Forest: 여러 개의 결정 트리(Decision Tree)를 조합한 앙상블 모델
- Regressor: 연속적인 수치 값을 예측하는 회귀 모델 (분류가 아닌 예측)

### 작동 원리:

1. 수백 개의 결정 트리를 생성 (n_estimators=300)
2. 각 트리는 랜덤하게 선택된 데이터와 특성으로 훈련
3. 예측 시 모든 트리의 결과를 평균내어 최종 예측값 산출

### 장점:

- 과적합 방지: 여러 트리의 평균으로 안정적인 예측
- 특성 중요도: 어떤 변수가 중요한지 자동으로 계산
- 빠른 학습: 병렬 처리 지원 (n_jobs=-1)

프로젝트 적용:
아파트 가격 예측에 적합한 이유는 비선형 관계(층수, 면적, 연식 등)를 잘 포착하고 안정적인 예측을
제공하기 때문입니다.
